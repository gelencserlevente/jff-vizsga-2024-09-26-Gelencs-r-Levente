...
function teszt05() {
    try {
        ...
        if (tesztElem[0].type == "text") {
            ujVisszajelzoSor("Vezetéknév tipusa", "Megfelelő");
        }
        else {
            ujVisszajelzoSor("Vezetéknév típusa", "NEM megfelelő");
        }
    }
    catch {
        ujVisszajelzoSor("Vezetéknév mező típusa", "HIBA");
    }
}
...
function teszt07() {
    try {
        let tesztElem = document.querySelectorAll("input");
        if (tesztElem[1].id == "firstName") {
            ujVisszajelzoSor("Keresztnév mező azonosítója", "Megfelelő");
        }
        else {
            ujVisszajelzoSor("Keresztnév mező azonosítója", "NEM megfelelő");
        }
    }
    catch {
        ujVisszajelzoSor("Keresztnév mező azonosítója", "HIBA");
    }
}
...
function teszt12() {
    try {
        let tesztElem = document.querySelectorAll("input");
        if (tesztElem[3].id == "password" && tesztElem[3].type == "password") {
            ujVisszajelzoSor("Jelszó mező azonosítója és típusa", "Megfelelő");
        }
        else {
            ujVisszajelzoSor("Jelszó mező azonosítója és típusa", "NEM megfelelő");
        }
    }
    catch {
        ujVisszajelzoSor("Jelszó mező azonosítója és típusa", "HIBA");
    }
}
